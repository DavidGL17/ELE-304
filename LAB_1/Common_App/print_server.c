...
bool PrintServerInitCommon() {
   ...
   osMemoryPoolAttr_t print_server_pool_attributes = {.name = "PrintServerPoolCM4"};
#else
	/* Define PrintServer memory pool attributes */
	osMemoryPoolAttr_t print_server_pool_attributes = { .name = "PrintServerPoolCM7", .mp_mem = pool_buf_cm7, .mp_size =
			SIZE_OF_POOL };
#endif
	...
	print_server_pool_id = osMemoryPoolNew(PS_POOL_AND_MESSAGE_QUEUE_SIZE, sizeof(print_server_block_format_t),
			&print_server_pool_attributes);
	if (print_server_pool_id == NULL)
		return false;

	/* Create PrintServer message queue */
	print_server_msg_qid = osMessageQueueNew(PS_POOL_AND_MESSAGE_QUEUE_SIZE, sizeof(uint32_t),
